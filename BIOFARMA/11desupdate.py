...
logger.info("Bot sedang diinisialisasi...")                        
...
# Conversation states
MENU, ASK_NUM_ACCOUNTS, INPUT_PHONES, INPUT_VERIFICATION_CODE, VCF, SELECT_ACCOUNT_DELETE, ADMIN_MENU, ADD_USER, CONFIRM_CONTINUE, VCF_SELECT, CHANGE_NUMBER, VCF_REUPLOAD, DELETE_USER_MENU, DELETE_SINGLE_USER, CONFIRM_DELETE_ALL, WAIT_FOR_NOTIFICATION_MESSAGE = range(16)
...
def handle_vcf_file(update: Update, context: CallbackContext) -> int:
    """
    🔥 FIXED: Handler VCF yang langsung auto-detect nomor yang digunakan
    """
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id
    
    logger.info(f"📁 handle_vcf_file called by user {user_id}")
    ...
    if update.message.document:
        ...
        logger.info(f"📄 File received: {document.file_name}")
        
        if not document.file_name.lower().endswith('.vcf'):
            update.message.reply_text("❌ File yang dikirim bukan file VCF. File harus berakhiran .vcf")
            return VCF
        
        # 🔥 PERBAIKAN UTAMA: Auto-detect nomor dari active session
        active_session = context.user_data.get('active_session')
        
        if not active_session:
            # Jika tidak ada active session, cek apakah ada existing sessions
            existing_sessions = get_existing_sessions()
            
            if not existing_sessions:
                update.message.reply_text(
                    "❌ Tidak ada nomor yang login.\n\n"
                    "Silakan login terlebih dahulu dengan memilih 'Login Akun Baru' di menu."
                )
                return start(update, context)
            
            # Jika ada multiple sessions, pilih yang pertama (atau bisa dikasih opsi)
            if len(existing_sessions) == 1:
                # Hanya 1 session, langsung pakai
                ...
                update.message.reply_text(
                    f"✅ Auto-detected nomor: {phone_number}\n"
                    f"📥 Mengunduh file VCF..."
                )
            else:
                # Multiple sessions, minta user pilih
                update.message.reply_text(
                    "📱 Anda memiliki beberapa nomor yang login.\n"
                    "Silakan pilih nomor yang akan digunakan:"
                )
        ...
        try:
            ...
            update.message.reply_text(f"❌ Gagal mengunduh file: {e}")
            return VCF
        
        # 🔥 LANGSUNG PROSES - TIDAK PERLU PILIH NOMOR LAGI!
        update.message.reply_text(
            f"🔄 Memproses file VCF untuk nomor {phone_number}...\n\n"
            f"⏱️ Proses ini mungkin membutuhkan waktu beberapa menit.\n"
            f"Anda dapat menghentikan proses dengan mengirim /stop"
        )
...
def process_vcf(filename, session_name, phone_number, context):
    ...
    if not contacts:
        context.bot.send_message(chat_id=chat_id, text="❌ Tidak ada kontak ditemukan dalam file VCF.")
        return
    
    context.bot.send_message(
        chat_id=chat_id, 
        text=f"📊 Total kontak: {len(contacts)}\n🔄 Memulai proses import..."
    )
    ...
    try:
        ...
        try:
            ...
            context.bot.send_message(chat_id=chat_id, text=f"❌ Connection failed: {str(e)}")
            return

        if not client.is_user_authorized():
            context.bot.send_message(chat_id=chat_id, text="❌ Not logged in. Please login first.")
            return
        ...
        for chunk_idx in range(total_chunks):
            ...
            context.bot.send_message(
                chat_id=chat_id, 
                text=f"💼 Processing batch {chunk_idx+1}/{total_chunks} (contacts {start_idx+1}-{end_idx})"
            )
            ...
            for rel_idx, (name, phone) in enumerate(chunk):
                ...
                try:
                    ...
                    if user:
                        context.bot.send_message(
                            chat_id=chat_id, 
                            text=f"✅ [{index}] {name} - {phone} Okeni."
                        )
                        consecutive_not_found = 0
                    else:
                        context.bot.send_message(
                            chat_id=chat_id, 
                            text=f"⚠️ [{index}] {name} - {phone} jelek"
                        )
                    ...
                    if consecutive_not_found >= MAX_CONSECUTIVE_NOT_FOUND:
                        ...
                        try:
                            ...
                            context.bot.send_message(
                                chat_id=chat_id,
                                text=f"⚠️ {MAX_CONSECUTIVE_NOT_FOUND} kontak berturut-turut tidak ditemukan.\n\n"
                    ...
                    context.bot.send_message(
                        chat_id=chat_id, 
                        text=f"❌ [{index}] Failed: {name}: {error_msg}"
                    )
                    ...
                    if any(keyword in error_msg.lower() for keyword in ["flood", "wait", "too many", "rate limit"]):
                        ...
                        context.bot.send_message(
                            chat_id=chat_id, 
                            text=f"⚠️ Rate limit detected. Waiting {wait_time} seconds..."
                        )
                        ...
                        while remaining > 0:
                            ...
                            if remaining > 0 and remaining % 60 == 0:
                                context.bot.send_message(
                                    chat_id=chat_id,
                                    text=f"⏳ Waiting {remaining} more seconds..."
                                )
        ...
        context.bot.send_message(chat_id=chat_id, text="✅ Process complete!")
            ...
            try:
                ...
                for attempt in range(3):
                    try:
                        context.bot.send_message(
                            chat_id=chat_id,
                            text="Or choose other option:",
                            reply_markup=reply_markup
                        )
                        logger.info(f"✅ Reupload buttons sent")
                        break
                    except Exception as e:
                        logger.error(f"❌ Error sending reupload buttons: {e}")
                        if attempt < 2:
                            time.sleep(1)
        ...
        context.bot.send_message(chat_id=chat_id, text="❌ VCF file not found.")
        logger.error(f"❌ VCF file not found: {filename}")
    except Exception as e:
        context.bot.send_message(chat_id=chat_id, text=f"❌ Error processing file: {str(e)}")
...
def change_number_for_vcf(query, context):
    """Change number for VCF processing"""
    chat_id = query.message.chat_id
    
    existing_sessions = get_existing_sessions()
    
    if 'current_vcf_process' in context.user_data:
        current_phone = context.user_data['current_vcf_process']['phone_number']
        available_sessions = [s for s in existing_sessions if s != current_phone]
    else:
        available_sessions = existing_sessions
    
    if not available_sessions:
        try:
            query.edit_message_text(text="❌ Tidak ada nomor lain yang login.")
        except:
            context.bot.send_message(chat_id=chat_id, text="❌ Tidak ada nomor lain yang login.")
        
        time.sleep(1)
        return start(None, context)
    
    keyboard = []
    for i, phone in enumerate(available_sessions[:10], 1):
        keyboard.append([KeyboardButton(f"{i}. {phone}")])
    keyboard.append([KeyboardButton("⬅️ Batal")])
    ...
    try:
        context.bot.send_message(
            chat_id=chat_id,
            text="📱 Pilih nomor baru:",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Error sending change number message: {e}")
    
    context.user_data['available_change_sessions'] = available_sessions
    context.user_data['is_changing_number'] = True


def handle_change_number_choice(update: Update, context: CallbackContext) -> int:
    """Handle number change choice"""
    text = update.message.text.strip()
    ...
    try:
        number_index = int(text.split(".")[0]) - 1
        available_sessions = context.user_data.get('available_change_sessions', [])
        
        if 0 <= number_index < len(available_sessions):
            new_phone = available_sessions[number_index]
            new_session = f"session_{new_phone}"
            ...
            # Update VCF process if exists
            if 'current_vcf_process' in context.user_data:
                context.user_data['current_vcf_process']['phone_number'] = new_phone
                context.user_data['current_vcf_process']['session_name'] = new_session
                
                update.message.reply_text(
                    f"✅ Nomor diganti ke: {new_phone}\n"
                    f"🔄 Melanjutkan proses..."
                )
                ...
                update.message.reply_text(
                    f"✅ Nomor aktif diganti ke: {new_phone}\n"
