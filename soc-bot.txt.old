...
# Telegram Security
ADMIN_IDS = {6872081104}  # Admin user IDs (full access)
...
LOG_FILE = os.path.join(LOG_DIR, "soc_bot.log")
...
def add_to_blacklist_db(ip: str, reason: str = "auto") -> bool:
    """Add IP to blacklist database with timestamp"""
    db = load_blacklist_db()
    if ip in db:
        return False
...
def add_to_blacklist(ip: str, reason: str = "auto") -> bool:
    """Add IP to blacklist"""
    ...
    add_to_blacklist_db(ip, reason)
    
    logger.info(f"BLACKLISTED | IP={ip} | REASON={reason}")
    return True
...
def remove_from_blacklist(ip: str) -> bool:
    ...
    logger.info(f"UNBLACKLISTED | IP={ip}")
    return True


def force_blacklist(ip: str, reason: str = "Manual/Anomaly") -> bool:
    ...
    logger.info(f"FORCE_BLACKLISTED | IP={ip} | REASON={reason}")
    return True
...
def force_blacklist_cli():
    ...
    ip = input(f"{Colors.GREEN}Enter IP (or 'back') > {Colors.RESET}").strip()
    ...
    if is_private_ip(ip):
        print(f"{Colors.YELLOW}[!] Private IP - tidak perlu blacklist{Colors.RESET}")
        input(f"{Colors.GREEN}Press Enter...{Colors.RESET}")
        return

    if ip in load_blacklist():
        print(f"{Colors.YELLOW}[!] IP sudah ada di blacklist{Colors.RESET}")
        input(f"{Colors.GREEN}Press Enter...{Colors.RESET}")
        return

    reason = input(f"{Colors.GREEN}Alasan (optional) > {Colors.RESET}").strip()
    if not reason:
        reason = "Manual/Anomaly"

    confirm = input(f"\n{Colors.YELLOW}Blacklist {ip}? (y/n) > {Colors.RESET}").strip().lower()
    if confirm != 'y':
    ...
    if force_blacklist(ip, reason):
        print(f"{Colors.GREEN}[+] {ip} berhasil di-blacklist{Colors.RESET}")
        print(f"{Colors.BLUE}[*] Auto-pushing to GitHub...{Colors.RESET}")
        result = git_push()
        print(f"{Colors.GREEN}  {result}{Colors.RESET}")
    else:
        print(f"{Colors.RED}[-] Gagal blacklist IP{Colors.RESET}")
    ...
    async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "menu":
            ...
            await query.edit_message_text(
                ...
                "Kirim IP yang ingin di-blacklist:",
                parse_mode="Markdown", reply_markup=cancel_keyboard()
            )
...
"""
            ...
            if not require_admin(update):
                return
            result = git_push()
            await query.edit_message_text(f"**Git Push**\n\n{result}", parse_mode="Markdown", reply_markup=admin_keyboard())
    ...
    async def message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if mode == "add_staff":
            ...
            if not require_admin(update):
                await update.message.reply_text("\u274c Admin only", reply_markup=admin_keyboard())
                return
        ...
        # Handle dump_range_end mode
            ...
            if text in load_blacklist():
                await update.message.reply_text(f"\u26a0\ufe0f `{text}` sudah di blacklist", parse_mode="Markdown", reply_markup=back_keyboard())
                return
            ...
            if is_private_ip(text):
                await update.message.reply_text(
                    "⚠️ IP private tidak perlu blacklist",
                    reply_markup=back_keyboard()
                )
                return
            ...
            if text in load_blacklist():
                await update.message.reply_text(
                    f"⚠️ `{text}` sudah ada di blacklist",
                    parse_mode="Markdown",
                    reply_markup=back_keyboard()
                )
                return

            if force_blacklist(text, "Manual/Anomaly - Telegram"):
                result = f"✅ **Force Blacklist Berhasil**\n\n"
                result += f"IP: `{text}`\n"
                result += f"Alasan: Manual/Anomaly\n\n"
                result += "💾 _Added to blacklist_"
                ...
                await update.message.reply_text(
                    "❌ Gagal menambahkan ke blacklist",
                    reply_markup=back_keyboard()
                )
